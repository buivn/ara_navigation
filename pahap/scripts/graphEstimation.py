...
class graphEstimate_server:
  ...
  def graphEstimate_response(self, data):
    if data.cmd:
      ...
      m_ver, m_edges = self.gCon_midBorder(centers, borPoint_matrix)
      
      # add more vertices and edges for cluster with only one or two neighbors
      g_ver, g_edges = self.add_verEd_cluster12Neigh(m_ver,m_edges,line_cooefs, clusterSet, neigh_matrix, centers)
      ...
      for i in range(len(route)):
        point = Point()
        point.x = g_ver[route[i]][0]
        point.y = g_ver[route[i]][1]
        point.z = 0.0
        points.append(point)
      ...
      for k in range(len(route)-1):
        ...
        del_x = 0.24*(v_x3 - v_x2)
        del_y = 0.24*(v_y3 - v_y2)
        add = random.uniform(-1.0,1.0)/55
        plt.arrow(v_x2, v_y2, del_x, del_y, head_width=0.02, width=0.003)
      ...
      # return graph_estimateResponse(True, None, None, None, None)      
  ...
  def add_verEd_cluster12Neigh(self, ver, ed, lines, clusters, n_matrix, cens):
    #get the cluster with one neighbor
    clus1Neigh = []
    insecSet = []
    vertices = copy.deepcopy(ver)
    edges = copy.deepcopy(ed)
    for i in range(len(n_matrix)):
      ...
      if sum == 1.0:
        clus1Neigh.append(i)
    for k in range(len(clus1Neigh)):
      insect = self.lineBound_intersect(lines[clus1Neigh[k]], clusters[clus1Neigh[k]])
      checkFar1 = True 
      checkFar2 = True 
      for h in range(len(ver)):
        # check if the intersection close to any vertices
        if self.dis_twoPoints([insect[0],insect[1]], ver[h]) < 0.2:
          checkFar1 = checkFar1 and False
          # break
        if self.dis_twoPoints([insect[2],insect[3]], ver[h]) < 0.2:
          checkFar2 = checkFar2 and False
          # break
      if checkFar1: # if it is far enough, add it to the vertex list
        vertices = np.append(vertices, [[insect[0],insect[1]]], axis=0)
        ver_index = 0
        # find the center point index
        for m in range(len(ver)):
          if (cens[clus1Neigh[k]][0] == ver[m][0]) and (cens[clus1Neigh[k]][1] == ver[m][1]):
            ver_index = m
        # add edge
        dis = self.dis_twoPoints([insect[0],insect[1]], cens[clus1Neigh[k]])
        edges = np.append(edges, [[ver_index, len(vertices)-1, dis]], axis=0)

      if checkFar2: # if it is far enough, add it to the vertex list
        vertices = np.append(vertices, [[insect[2],insect[3]]], axis=0)
        ver_index = 0
        # find the center point index
        for m in range(len(ver)):
          if (cens[clus1Neigh[k]][0] == ver[m][0]) and (cens[clus1Neigh[k]][1] == ver[m][1]):
            ver_index = m
        # add edge
        dis = self.dis_twoPoints([insect[2],insect[3]], cens[clus1Neigh[k]])
        edges = np.append(edges, [[ver_index, len(vertices)-1, dis]], axis=0)
