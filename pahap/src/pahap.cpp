...
void PahaP::initServer()
{
  pahap_getPcl_ser_ = nh_.advertiseService("pointcloud_cmd", &PahaP::check_and_save, this);
...
void PahaP::pointcloudCallback(const sensor_msgs::PointCloud2ConstPtr& msg){   
  // Convert to PCL data type
  pcl::fromROSMsg(*msg, *pclTopic_input_);
}

bool PahaP::boundaryEstimate(pahap::pcl_boundary::Request &req,
                            pahap::pcl_boundary::Response &res){
  if (req.cmd) {
    // SvEv3f ppoints1, ppoints2, ppoints3, ppoints4, ppoints5;
    // SvEv2f bound1, bound2, bound3, bound4, bound5;
    
    for (int j=0; j< req.clusterSet.size(); j++) {
      Eigen::Vector3f point;
      SvEv3f ppoints;
      SvEv2f bound;
      pahap::cluster clus;
      for (int i = 0; i < req.clusterSet[j].cluster.size(); i++) {
        point[0] = req.clusterSet[j].cluster[i].x;
        point[1] = req.clusterSet[j].cluster[i].y;
        point[2] = req.clusterSet[j].cluster[i].z;
        ppoints.push_back(point);
        // std::cout << "number of data in each cluster: " << ppoints.size() << std::endl;
        // std::cout << "number of data in each cluster: " << req.clusterSet[j].cluster.size() << std::endl;
      }
      // get the maximum and minimum coordinate of 2D data
      EvXf max_min_xy = det_maxmin_xy(ppoints);
      // std::cout << max_min_xy << std::endl;
      // get the set of boundary points
      bound = est_2DBound(ppoints, max_min_xy, 0.02); // slicing_factor =0.02
      int count =0;
      for (int it = 0; it < bound.size(); ++it) {
        ...
        point.y = bound[it][1];
        point.z = 0;
        clus.cluster.push_back(point);
        count++;
      }
      res.clusterSet.push_back(clus);
      res.numofData.push_back(count);
    }
    res.test = true;    
    return true;
  }
...
bool PahaP::navigate(pahap::pointcloud_nav::Request &req,
                            pahap::pointcloud_nav::Response &res){
  if (req.cmd) {

    if (req.topic) copyPointCloud(*pclTopic_input_, *pcl_input_);
    else copyPointCloud(*pclFile_input_, *pcl_input_); 
    // display_pcd(pcl_input_, 0, "Original Data");
    pclXYZRGB::Ptr pcl_pass (new pclXYZRGB);
    // pcl_pass = passthrough_filter(pcl_input_);
    // pcl_pass = passthrough_filter(pclTopic_input_);
    pcl_pass = downsampling(pcl_input_);
    // display_pcd(pcl_pass, 0, "Downsampling Data");
    pclXYZRGB::Ptr structure_planes (new pclXYZRGB);
    structure_planes = est_multPlane(pcl_pass);
    // pcl_pass = est_multPlane(pcl_pass);
    // display_pcd(structure_planes, 0, "Structure Planes");
    // pointCloudCb(structure_planes);
    pcl_pass = pcl_frameTransform(structure_planes);
    // display_pcd(pcl_pass, 0, "transformed Planes");

    std::vector<projectedPoint> ppoints = project_pclData(pcl_pass);
    
    int count = 0;
    for (std::vector<projectedPoint>::iterator it = ppoints.begin(); it != ppoints.end(); ++it) {
        geometry_msgs::Point point;
        point.x = (*it).x;
        point.y = (*it).y;
        point.z = 0;
...
bool PahaP::check_and_save(pahap::pointcloud_cmd::Request &req,
                            pahap::pointcloud_cmd::Response &res){
  if (req.cmd) {
    ...
    std::stringstream ss;
    ss << req.path + req.num_name+".pcd";
    writer.write<pcl::PointXYZRGB> (ss.str(), *pcl_input_, false);
    // pcd_write_unPcl("Plane", pcl_pass);
    ...
    // display_clofarPair(pcl_pass, closFart, cen);

    // geometry_msgs::Pose pose1;
    // pose1 = est_Pose1(cen, closFart, normal);
...
pclXYZRGB pcd_read(const std::string& filename) {
  ...
  //         << " data points from the pcd file with the following fields: " << std::endl;
  return cloud;
}

//passThrough filter
pclXYZRGB::Ptr PahaP::passthrough_filter(pclXYZRGB::Ptr cloud) {
...
pclXYZRGB::Ptr PahaP::downsampling(pclXYZRGB::Ptr cloud) {
    pclXYZRGB::Ptr cloud_filtered (new pclXYZRGB);
...
pclXYZRGB::Ptr PahaP::est_plane(pclXYZRGB::Ptr cloud) {
...
void PahaP::pointCloudCb(pclXYZRGB::Ptr cloud){

    // Publish points
    sensor_msgs::PointCloud2 cloud_publish;
    pcl::toROSMsg(*cloud, cloud_publish);
    ros::Rate rate(10);
    // cloud_publish.header.seq = ;
    cloud_publish.header.stamp = ros::Time::now();
    cloud_publish.header.frame_id = "processed_pcl";
    for (int i =0; i < 10000; i++) {
      cloud_publish.header.stamp = ros::Time::now();
      pahap_pcl_pub_.publish(cloud_publish);
      ros::spinOnce();
      rate.sleep();
    }
...
pclXYZRGB::Ptr PahaP::est_multPlane(pclXYZRGB::Ptr cloud) {
    
    pclXYZRGB::Ptr cloud_left (new pclXYZRGB);
    // vector of pclXYZRGB::Ptr for multiple plane
    std::vector <pclXYZRGB::Ptr, Eigen::aligned_allocator <pclXYZRGB::Ptr> > cloud_planes;
    // vector <pclXYZRGB::Ptr> cloud_planes;
    pcl::ModelCoefficients::Ptr coefficients (new pcl::ModelCoefficients);
    pcl::PointIndices::Ptr inliers (new pcl::PointIndices);  // variable to store the point cloud belong to the desired object
    // Create the segmentation object -> filter the desired point belong to the object
    pcl::SACSegmentation<pcl::PointXYZRGB> seg;
    ...
    while (cloud->points.size() > 0.3*nr_points)
    {
      ...
      // std::cout << "check ing asdf : " << count << std::endl;
      // Segment the largest planar component from the remaining cloud
      seg.setInputCloud(cloud);
      // seg.segment() result in the inliers point and coefficients
      seg.segment (*inliers, *coefficients); //save all points belong to the plane to inliners
      ...
      pclXYZRGB::Ptr cloud_plane (new pclXYZRGB);
      
      // copy the index->cloud to a plane
      for (int i=0; i<inliers->indices.size();i++){
          // Get a plane
          cloud_plane->points.push_back(cloud->points[inliers->indices[i]]);
      }
      // display_pcd(cloud_plane, 0, "test");

      //clustering the data by KdTree object for the search method of the extraction
      ...
      //select with largest point cloud data
      int max = 0;
      for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
        if (it->indices.size() > max)
          max = it->indices.size();
      // std::cout << "The value of max" << max << std::endl;
      ...
      for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it)
      {             
        if (max == it->indices.size()) {
          // copy all the point cloud of the cluster to a new point cloud data
          ...
          // display_pcd(cloud_cluster, 0, "Displaying the cluster with most cloud");
        }
        // max = 0;
      }

      // calculate distance between a point (origin) and a plane (just determined)
      Eigen::Vector4f pp(0.0,0.0, 0.0, 1); // (p.x,p.y,p.z,1)
      
      float nom = std::abs(pp[0]*coefficients->values[0] + pp[1]*coefficients->values[1] + pp[2]*coefficients->values[2] +coefficients->values[3]);
      float dem = std::sqrt(std::pow(coefficients->values[0],2) + std::pow(coefficients->values[1],2) + std::pow(coefficients->values[2],2));
      
      float distance_to_plane = nom/dem;
      // std::cout << distance_to_plane << std::endl;
      distances.push_back(distance_to_plane);

      // save the plane cloud to the vector of planes
      cloud_planes.push_back(cloud_cluster);
      ...
      extract.setIndices(inliers);
      // set whether the indices should be returned, or all points_except_the indices
      extract.setNegative (true); //true -> return the data left (not belong to the indices)
      // save the result to the cloud_p
    ...
    pclXYZRGB::Ptr plane_return (new pclXYZRGB);
    for (int i = 0; i < cloud_planes.size(); i++)
      if (plane_number == i){
        plane_return.swap(cloud_planes[i]);
      }

    return plane_return;
}

pclXYZRGB::Ptr PahaP::pcl_frameTransform(pclXYZRGB::Ptr cloud) {
  pclXYZRGB::Ptr transformed_cloud (new pclXYZRGB);


  Eigen::Affine3f transform_matrix = Eigen::Affine3f::Identity();
  Em4f hete_matrix;
  // Em4f hete_matrix; = eulerAngletoRotationMatrix(x, y, z, roll, pitch, yaw);
  hete_matrix = eulerAngletoRotationMatrix(-0.397, 0.0, 0.416, -2.147192, 0.0, 1.570796);
  transform_matrix.matrix() = hete_matrix;
  
  // Define a translation of 2.5 meters on the x axis.
  // transform_matrix.translation() << 2.5, 0.0, 0.0;

  // The same rotation matrix as before; theta radians around Z axis
  // float theta = M_PI/4; // The angle of rotation in radians
  // transform_matrix.rotate (Eigen::AngleAxisf (theta, Eigen::Vector3f::UnitZ()));

  // Eigen::Vector3f axis(-0.0314789, 0.0207277, -0.999289);
  // pcl::PointXYZRGBNormal center;
  // center.x = -0.00468551; center.y = 0.00173406; center.z = -0.442;

  // Eigen::Quaternionf rotAxe;
  // rotAxe.setFromTwoVectors(Eigen::Vector3f(0, 0, 1), axis);
  // Eigen::Affine3f rotation = Eigen::Affine3f::Identity();
  // rotation.prerotate(rotAxe);

  // Eigen::Affine3f translation = Eigen::Affine3f::Identity();
  // translation.translation() << center.getArray3fMap();
  // pcl::transformPointCloud(*cloud, *transformed_cloud, translation*rotation);
...
// this function projected the data on XOY direction.
std::vector<projectedPoint> PahaP::project_pclData(pclXYZRGB::Ptr cloud){

  std::vector<projectedPoint> points;
  projectedPoint point;
  // pclXYZRGB::iterator it;
  // for (it= cloud->begin(); it!= cloud->end(); it++)
  for (int i= 0; i < cloud->points.size(); ++i){
    // cloud3->push_back (pcl::PointXYZ (it->x, it->y, it->z));
...
Ev3f PahaP::est_planeNormal(pclXYZRGB::Ptr cloud) {
...
EvXf PahaP::det_MaxMinxyz(pclXYZRGB::Ptr cloudp) {
...
// get the maximum and minimum coordinate of 2D data
EvXf PahaP::det_maxmin_xy(SvEv3f data) {
    Eigen::VectorXf max_min(4);
...
// get the set of boundary points
SvEv2f PahaP::est_2DBound(SvEv3f data, EvXf max_minxy, float slicing_factor)  // slicing_factor =0.02
{
    
  std::vector<Eigen::Vector2f> twoDboundary;
  Eigen::Vector2f point1(0,0), point2(0,0);
  float start, distance1, distance2, max_distance;
  Eigen::VectorXf maxmin(4);
  for (int i =0; i < 4; i++)
  ...
  for (int axis = 0; axis < 2; axis++)
  {
    ...
    {   
      for (int i = 0; i< data.size(); i++)
      {
        // select the right distance
        float dis_xy=0;
        if (axis == 0) dis_xy = data[i][0] - start;
        if (axis == 1) dis_xy = data[i][1] - start;
        ...
        if ((dis_xy < slicing_factor/2) and (dis_xy > -slicing_factor/2))
        {
          ...
          else if (count_check ==1) // this is the second point
          {
            point2[0] = data[i][0];
            point2[1] = data[i][1];
            max_distance = pow((point2[0]-point1[0]),2)+pow((point2[1]-point1[1]),2);
          }
          else {
            distance1 = pow((point1[0]-data[i][0]),2)+pow((point1[1]-data[i][1]),2);
            distance2 = pow((point2[0]-data[i][0]),2)+pow((point2[1]-data[i][1]),2);
            if (distance2 < distance1)
              if (distance1 > max_distance) {
                max_distance = distance1;
                point2[0] = data[i][0];
      ...
      // check if there is points with common coordinate
      bool commonCoor1 = false, commonCoor2 = false;
      for (int j = 0; j < twoDboundary.size(); j++) {
      ...
      if (not commonCoor1) twoDboundary.push_back(point1);
      if (not commonCoor2) twoDboundary.push_back(point2);
      start += slicing_factor/2;
      point1[0] = 0.0;
      point1[1] = 0.0;
      point2[0] = 0.0;
...
EvXf PahaP::est_clofarPoints_toCentroid(SvEv3f B_Point, Ev4f centroid) {
...
// approximate the area by closest and farthest point pair
float PahaP::cal_PlaneArea(const EvXf clos_fart)
{
...
geometry_msgs::Pose PahaP::est_Pose1(Ev4f centroid, EvXf clos_fart, Ev3f ave_normal)
{
...
Em4f PahaP::convert_PosetoRotationMatrix(geometry_msgs::Pose robot_pose)
{
    // geometry_msgs::Pose robot_pose;
...
// convert euler angles to heterousgeneous matrix
Em4f PahaP::eulerAngletoRotationMatrix(float x, float y, float z, float roll, float pitch, float yaw)
{
    // geometry_msgs::Pose robot_pose;
    Eigen::Matrix4f hete_matrix;
    ...
    for (int i = 0; i < 3; i++) {
      for (int j = 0; j < 3; j++) {
        hete_matrix(i,j) = transformation(i,j);
      }
    }
    hete_matrix(0,3) = x;
    hete_matrix(1,3) = y;
...
EmXf PahaP::est_5closPoints_toCentroid(SvEv3f B_Point, Ev4f centroid)
{
...
EmXf PahaP::det_5rectangle(EmXf fiPoint, Ev4f centroid, Ev3f normal, float l, float w)
{
...
SvPclIndices PahaP::extract_objects(pclXYZRGB::Ptr cloud) {
    // Creating the KdTree object for the search method of the extraction
    pcl::search::KdTree<pcl::PointXYZRGB>::Ptr tree (new pcl::search::KdTree<pcl::PointXYZRGB>);
    tree->setInputCloud (cloud);
    SvPclIndices cluster_indices;  // create the indices vector for clusters in the point cloud
    pcl::EuclideanClusterExtraction<pcl::PointXYZRGB> ec; // store the cluster extraction of point cloud
    ec.setClusterTolerance (0.02); // radius = 2cm of the sphere to determine the neighbor points
    ec.setMinClusterSize (70);  // min number of point in a cluster
    ec.setMaxClusterSize (25000);  // max number of point in a cluster
    ec.setSearchMethod(tree);  // the search method is tree
...
Em4f PahaP::est_objectCentroids(pclXYZRGB::Ptr cloud, SvPclIndices c_indices, int nCluster) {
  ...
  for (SvPclIndices::const_iterator it = c_indices.begin (); it != c_indices.end (); ++it) {     
      pcl::compute3DCentroid(*cloud, *it, centroid);
      centroid_vector.row(count) = centroid; // save the centroid to the centroi_vector     
      ++count;
  }
  return centroid_vector;
}

pclXYZRGB::Ptr PahaP::integrate_clusters(pclXYZRGB::Ptr cloud, SvPclIndices cluster_indices) {
    pclXYZRGB::Ptr com_cluster (new pclXYZRGB); 
    // go to each point cloud cluster in the cluster vector - cluster_indices
    for (SvPclIndices::const_iterator it = cluster_indices.begin (); it != cluster_indices.end (); ++it)
    {    
...
void PahaP::display_pcd(const pclXYZRGB::Ptr cloud, int clus_order, const std::string& titlename) {
    int v1(0);
...
void PahaP::display_centroid_multCoor(pclXYZRGB::Ptr cloud, EmXf& centroid3D)
{
...
void PahaP::display_normals(const pclXYZRGB::Ptr cloud, const pclNormal::Ptr normals) {
...
void PahaP::display_planeNormals(pclXYZRGB::Ptr cloud, Ev3f normals, Ev4f centroid) {
  pcl::visualization::PCLVisualizer::Ptr viewer (new pcl::visualization::PCLVisualizer ("Plane Normals"));
  viewer->setBackgroundColor (0, 0, 0);
  // Add text to the screen
  //viewer->addText("Radius: 0.002", 50, 50, "v1 text", v1);
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> single_color(cloud, 0, 255, 0);
...
void PahaP::display_clofarPair(pclXYZRGB::Ptr cloud, EvXf closfart, Ev4f centroid) {
  pcl::visualization::PCLVisualizer::Ptr viewer (new pcl::visualization::PCLVisualizer ("Closest & Farthest Points to Center"));
  viewer->setBackgroundColor (0, 0, 0);
  // Add text to the screen
  //viewer->addText("Radius: 0.002", 50, 50, "v1 text", v1);
  pcl::visualization::PointCloudColorHandlerCustom<pcl::PointXYZRGB> single_color(cloud, 0, 255, 0);
...
void PahaP::display_targetCoordinate(pclXYZRGB::Ptr cloud, Em4f RotMatrix) {
...
void PahaP::display_pointSet_asPointCloud(SvEv3f setOfPoint) {
  pclXYZ cloud;
...
void PahaP::draw_Rectangles(EmXf fiRect, pclXYZRGB::Ptr cloudp) {
...
void PahaP::pcd_write_unPcl(const std::string& filename, pclXYZRGB::Ptr cloud) {  
  pcl::PCDWriter writer;
  std::stringstream ss;
  ss << "prep_"+filename+".pcd";
  writer.write<pcl::PointXYZRGB> (ss.str(), *cloud, true); //*
...
int main(int argc, char **argv)
{
  ...
  std::string filename;
  n_para.param("fileAddress", filename, std::string("/home/buivn/bui_ws/src/pahap/src/L_shape.pcd"));
